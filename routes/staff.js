...
// Staff Download Report - same as student but for staff verification
router.get('/download-result/:admission_number/:term/:academic_year', async (req, res) => {
    try {
        ...
        const studentRes = await db.query('SELECT id, first_name, last_name FROM students WHERE admission_number = $1', [admission_number]);
        if (studentRes.rows.length === 0) {
        ...
        // Use the same PDF generation as student download-result
        ...
        doc.on('end', () => {
            ...
            res.set({
                ...
                'Content-Disposition': `attachment; filename="Report_${student.first_name}_${student.last_name}_${term}_${academic_year}.pdf"`
            }).send(pdf);
        });

        // Header
        doc.fillColor('#1a5f3f').fontSize(22).font('Helvetica-Bold')
            .text('ISLAMIC SCHOOL MANAGEMENT SYSTEM', { align: 'center', width: 500 });
        doc.fontSize(10).text('Official Academic Report - Staff Verification', { align: 'center', width: 500 });
        doc.moveDown(0.5).moveTo(50, doc.y).lineTo(550, doc.y).strokeColor('#1a5f3f').lineWidth(2).stroke();

        // Student Info
        doc.moveDown(1);
        doc.fillColor('#000').fontSize(12).font('Helvetica-Bold').text('Student:', 50, doc.y);
        doc.font('Helvetica').text(`${student.first_name} ${student.last_name}`, 120, doc.y);
        doc.text(`Admission: ${admission_number}`, 50, doc.y + 20);
        doc.text(`Term: ${term}`, 300, doc.y);
        doc.text(`Year: ${academic_year}`, 300, doc.y + 20);

        doc.moveDown(2);

        // Results Table
        const tableTop = doc.y, rowHeight = 25;
        doc.rect(50, tableTop, 500, rowHeight).fill('#1a5f3f');
        doc.fillColor('#fff').fontSize(10).font('Helvetica-Bold')
            .text('SUBJECT', 60, tableTop + 7)
            .text('SCORE', 300, tableTop + 7, { width: 50, align: 'center' })
            .text('GRADE', 380, tableTop + 7, { width: 50, align: 'center' })
            .text('REMARK', 460, tableTop + 7, { width: 80, align: 'center' });

        let y = tableTop + rowHeight;
        results.forEach((r, i) => {
            if (i % 2) doc.rect(50, y, 500, rowHeight).fill('#f9f9f9');
            const remark = r.grade === 'A' ? 'Excellent' : r.grade === 'B' ? 'Very Good' : r.grade === 'C' ? 'Good' : r.grade === 'D' ? 'Fair' : r.grade === 'E' ? 'Pass' : 'Fail';
            doc.fillColor('#000').fontSize(10).font('Helvetica')
                .text(r.subject, 60, y + 7)
                .text(r.score, 300, y + 7, { width: 50, align: 'center' })
                .text(r.grade, 380, y + 7, { width: 50, align: 'center' })
                .text(remark, 460, y + 7, { width: 80, align: 'center' });
            y += rowHeight;
        });

        doc.moveTo(50, y).lineTo(550, y).stroke('#aaa');

        // Summary
        const summaryY = y + 30;
        doc.rect(350, summaryY, 200, 60).stroke('#1a5f3f');
        doc.rect(350, summaryY, 200, 20).fill('#1a5f3f');
        doc.fillColor('#fff').font('Helvetica-Bold').fontSize(12).text('SUMMARY', 350, summaryY + 6, { width: 200, align: 'center' });
        doc.fillColor('#000').fontSize(10)
            .text('Total Score:', 360, summaryY + 30)
            .font('Helvetica-Bold').text(totalScore.toFixed(1), 480, summaryY + 30, { align: 'right', width: 60 })
            .text('Average:', 360, summaryY + 45)
            .font('Helvetica-Bold').text(`${averageScore}%`, 480, summaryY + 45, { align: 'right', width: 60 });

        // Footer
        const footerY = 750;
        doc.lineWidth(1).moveTo(50, footerY).lineTo(550, footerY).stroke();
        doc.fontSize(9).fillColor('#666').text('Generated by Islamic School System | Official Document', 50, footerY + 10, { width: 500, align: 'center' });

        doc.end();
    } catch (err) {
        console.error('Staff download error:', err);
...
router.post('/contact-messages/:id/reply', async (req, res) => {
    try {
        ...
        await sendEmail(original.email, `Re: ${original.subject}`, `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    ...
                    <div style="background-color: #f9f9f9; padding: 30px; border: 1px solid #ddd;">
                        ...
                        <div style="background-color: #fff; padding: 15px; border-left: 4px solid #1a5f3f;">
            ...
            from: `Islamic School <${process.env.EMAIL_FROM || process.env.EMAIL_USER || 'onboarding@resend.dev'}>`
        });
